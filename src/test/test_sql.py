...
from lib.sql import table_multimerge as sql_table_multimerge
from lib.utils import table_multimerge as pandas_table_multimerge
...
class TestSql(ProfiledTestCase):
    def _compare_dataframes_equal(self, df1: DataFrame, df2: DataFrame) -> None:
        records1 = _dataframe_records_iterator(df1)
        records2 = _dataframe_records_iterator(df2)
    ...
    def test_table_merge(self):
        ...
        with TemporaryDirectory() as workdir:
            ...
            with create_sqlite_database(db_file=sqlite_file) as conn:
                ...
                expected = test_data_left.merge(test_data_right, on=["col1"], how="inner")

                # Merge and output as an iterable
                result1 = DataFrame.from_records(
                    table_merge(conn, table_name_left, table_name_right, on=["col1"], how="inner")
                )
    ...
    def test_table_multimerge(self):
        ...
        with TemporaryDirectory() as workdir:
            ...
            with create_sqlite_database(db_file=sqlite_file) as conn:
                ...
                expected = pandas_table_multimerge(
                    [test_data_1, test_data_2, test_data_3], on=["col1"], how="inner"
                )

                # Merge and output as an iterable
                result1 = DataFrame.from_records(
                    sql_table_multimerge(
                        conn, [table_name_1, table_name_2, table_name_3], on=["col1"], how="inner"
                    )
                )
                self._compare_dataframes_equal(result1, expected)
